    ...
    <div className="rounded-2xl overflow-hidden border border-border bg-background shadow-sm hover:shadow-lg transition-all duration-300 group flex flex-col">
      <div className="relative overflow-hidden" style={{ height: "200px" }}>
        ...
        <div className="absolute top-3 left-3" style={{ zIndex: 5 }}>
          {property.status === "active" ? (
            <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-full bg-primary/90 text-white uppercase tracking-wide">
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className="flex items-center gap-1 text-muted-foreground mb-3">
          <MapPin className="w-3 h-3 flex-shrink-0" style={{ color: "hsl(var(--primary))" }} />
        ...
        <div className="grid grid-cols-2 gap-2 mb-3">
          <div className="rounded-lg p-2 text-center" style={{ background: "hsl(var(--primary) / 0.07)", border: "1px solid hsl(var(--primary) / 0.15)" }}>
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-0.5">Price</p>
            <p className="text-sm font-bold" style={{ color: "hsl(var(--primary))" }}>{formatPrice(property.price)}</p>
          </div>
          <div className="rounded-lg p-2 text-center" style={{ background: "hsl(var(--primary) / 0.07)", border: "1px solid hsl(var(--primary) / 0.15)" }}>
        ...
        <div className="flex gap-2 mt-auto">
          <button
            ...
            onMouseEnter={e => { const el = e.currentTarget; el.style.background = "hsl(var(--primary))"; el.style.color = "white"; }}
            onMouseLeave={e => { const el = e.currentTarget; el.style.background = "transparent"; el.style.color = "hsl(var(--primary))"; }}
          >
    ...
    <div className="min-h-screen py-8" style={{ background: "hsl(40,33%,98%)" }}>
      ...
      <div className="container max-w-7xl mx-auto px-4">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8 animate-fade-in">
          <div>
            <p className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: "hsl(var(--primary))" }}>Property Management</p>
            <h1 className="font-display text-3xl md:text-4xl font-bold text-foreground">Manage Properties</h1>
            <div className="h-1 w-16 rounded-full mt-2" style={{ background: "linear-gradient(90deg, hsl(var(--primary)), hsl(var(--accent)))" }} />
          </div>
          <button
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
          {[
            ...
            <div key={i} className="rounded-xl p-4 text-center border" style={{ background: "white", borderColor: "hsl(var(--primary) / 0.15)" }}>
              <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-1">{s.label}</p>
              <p className="font-display text-2xl font-bold" style={{ color: "hsl(var(--primary))" }}>{s.value}</p>
            </div>
          ))}
        </div>
        ...
        {getPropertiesLoading && (
          <div className="text-center py-16">
            <div className="inline-block w-8 h-8 border-2 rounded-full animate-spin" style={{ borderColor: "hsl(var(--primary))", borderTopColor: "transparent" }} />
